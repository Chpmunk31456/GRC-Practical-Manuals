...
- [x] Authoritative-source verification completed on 2026-08-26 with exact-source-state watch retained for publication-candidate reverification.
- [x] Dedicated Manual 05 QA passes on the current controlled build; any later material/control change requires a fresh exact-head run.
- [x] Technical and editorial source review completed and recorded in `qa/TECHNICAL_EDITORIAL_REVIEW_2026-08-26.md`.
- [x] `es-419` and `pt-BR` localization drafts completed from the controlled English master.
- [ ] Complete human semantic/terminology review of the `es-419` and `pt-BR` editions.
- [x] Complete educational graphics and accessible source-level explanations (three controlled Mermaid graphics with text equivalents).
- [ ] Complete DOCX/PDF generation and page-by-page accessibility/visual QA.
- [ ] Complete repository/security release audit at the exact final candidate.
