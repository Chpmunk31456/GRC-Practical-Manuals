#!/usr/bin/env python3
"""Safe runner for Manual 12 CCPA/CPRA publication generation.

Prevents inherited Manual 11/10/09/07/06 publication bindings from reading or
relabeling the wrong manual while preserving California privacy/legal,
semantic, accessibility, provenance, security, and human-review gates.
"""
...
def find_localized_chapters(language: str):
    ...
    used: list[str] = []
    for path in sorted(manual12.source_dir(language).glob("*.md")):
        if path.name in {"RUTAS_DE_IMPLEMENTACION_MANUAL_12.md", "CAMINHOS_DE_IMPLEMENTACAO_MANUAL_12.md"}:
            continue
        found = manual12.core.split_chapters(path.read_text(encoding="utf-8"))
        if not found:
            continue
        ...
        used.append(str(path.relative_to(manual12.ROOT)))
...
# Bind Manual 12 directly to the raw renderer retained before manual-specific
# relabeling wrappers are applied, preventing inherited manual labels.
manual12._base_render = hipaa06._base_render
manual12._base_alt = hipaa06._base_alt

manual12.find_localized_chapters = find_localized_chapters
manual12.core.find_localized_chapters = find_localized_chapters
manual12.base.find_localized_chapters = find_localized_chapters
...
if __name__ == "__main__":
    raise SystemExit(manual12.core.main())
